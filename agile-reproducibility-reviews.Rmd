---
title: "Reproducibility Review AGILE 2023"
author: "Daniel Nüst"
...
This document includes scripts and text analysis to support the reproducibility review at the [AGILE conference 2023](https://agile-online.org/conference-2023), which is organised by a team from TU Delft, the Netherlands.
...
The code of this document is published on GitHub in the repository [reproducible-agile/reviews-2023](https://github.com/reproducible-agile/reviews-2023), where you can inspect the R code in the file `agile-reproducibility-reviews.Rmd` and find instructions for reproducing the workflow.
...
submission_table <- rvest::html_table(x = submission_table_full,
                                      fill = TRUE)
names(submission_table) <- c("id", "authors", "title", "information", "paper", "assigment", "update"
                             ,"type"
                             ,"time"
                             ,"decision"
                             )
...
pages <- lapply(submission_table$file, function(f) {
  if(is.na(f))
    NA
  else pdftools::pdf_info(f)$pages
})
...
if (params$private_info) {
  ...
  submissions_and_reviews <- "https://drive.google.com/drive/folders/1CvcrOaooQK_2rFKU2v0MGQZmiLfTgk6S"
...
googlesheets4::range_read(
  ...
  write.csv("AGILE-Reproducibility-Review_report-DOIs.csv", row.names = FALSE)
```
...
2. Take a look at the [review report template](https://github.com/reproducible-agile/reviews-2023/blob/master/report-template/reproreview-template.Rmd) - even if you're not using it, it gives you guidance
...
7. Add a new component to the [OSF project for 2023 reproducibility reviews](https://osf.io/2k56f/)
...
I'm contacting you as the corresponding author of the paper "<TITLE>" submitted to AGILE 2023.
...
AGILE Reproducibility Committee Chair 2023
```
...
AGILE Reproducibility Committee 2023
```
...
congratulations to the acceptance of your submission "TITLE" as a full paper at the AGILE conference 2023.
